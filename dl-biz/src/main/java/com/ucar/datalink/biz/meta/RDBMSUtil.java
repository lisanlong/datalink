...
public class RDBMSUtil {
    ...
    private static final String ORACLE_URL = "jdbc:oracle:thin:@{0}:{1}:ORCL";
        ...
        try {
            ...
            if (table != null) {
                ...
                if (primaryCols.length > 1) {
                    ...
                    String res_minId=ObjectUtils.toString(result.get("minId"), "");
                    String res_maxId=ObjectUtils.toString(result.get("maxId"), "");
                    String res_count=ObjectUtils.toString(result.get("count"), "");

                    checkResult.setFromMinId(Long.valueOf(res_minId==""?"0":res_minId));
                    checkResult.setFromMaxId(Long.valueOf(res_maxId==""?"0":res_maxId));
                    checkResult.setFromDsCount(Long.valueOf(res_count==""?"0":res_count));
            ...
            if (table != null) {
                ...
                if (primaryCols.length > 1) {
                    ...
                    String sql = MessageFormat.format("select min({0}) as minId,max({0}) as maxId,count({0}) as count from {1} where {0}>={2} and {0}<={3}", primaryCols[0].getName(), tableName,
                            startId.toString(), endId.toString());
                    Map<String, Object> result = jdbcTemplate.queryForMap(sql);
                    String tar_minId=ObjectUtils.toString(result.get("minId"), "");
                    String tar_maxId=ObjectUtils.toString(result.get("maxId"), "");
                    String tar_count=ObjectUtils.toString(result.get("count"), "");
                    checkResult.setToMinId(Long.valueOf(tar_minId==""?"0":tar_minId));
                    checkResult.setToMaxId(Long.valueOf(tar_maxId==""?"0":tar_maxId));
                    checkResult.setToDsCount(Long.valueOf(tar_count==""?"0":tar_count));
                }
            } else {
